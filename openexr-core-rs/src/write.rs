...
impl From<WriteMode> for sys::exr_default_write_mode_t {
    fn from(mode: WriteMode) -> Self {
        match mode {
            WriteMode::Directly => sys::exr_default_write_mode::EXR_WRITE_FILE_DIRECTLY,
            WriteMode::TempFile => sys::exr_default_write_mode::EXR_INTERMEDIATE_TEMP_FILE,
        }
    }
}
...
impl Writer {
    ...
    pub fn start_with_init_and_options(
        ...
        unsafe { sys::exr_start_write(&mut context, filename.as_ptr(), default_mode, &initalizer) }
            .ok()?;
...
impl ContextOptions for WriteOptions {
    ...
    fn with_disable_chunk_reconstruction(mut self) -> Self {
        self.context_flags = self.context_flags.with_disable_chunk_reconstruction();
        self
    }
    ...
    fn without_disable_chunk_reconstruction(mut self) -> Self {
        self.context_flags = self.context_flags.without_disable_chunk_reconstruction();
        self
    }
